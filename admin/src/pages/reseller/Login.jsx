...
export default function ResellerLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.post('/api/reseller/login', {
        phone: phone.trim(),
        password,
      })

      if (response.data.success) {
        login(response.data.data.user, response.data.data.token, 'reseller')
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-gradient-to-br from-emerald-900 via-teal-900 to-emerald-900 flex items-center justify-center p-4">
      ...
      <div className="absolute top-1/2 left-1/2 w-64 h-64 bg-cyan-500/5 rounded-full -translate-x-1/2 -translate-y-1/2 blur-3xl"></div>
        ...
        <div className="text-center mb-8">
          ...
          <div className="flex items-center justify-center gap-2 mt-2">
            ...
            <div className="flex items-center gap-2 px-3 py-1 bg-emerald-50 rounded-full">
              ...
              <p className="text-sm font-medium text-emerald-700">Revendeur</p>
            </div>
