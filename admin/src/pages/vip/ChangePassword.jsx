...
export default function VIPChangePassword() {
  const { user } = useVIPAuth()
  ...
  const submit = async (e) => {
    ...
    setLoading(true)
    try {
      const res = await api.post('/api/vip/change-password', { oldPassword, newPassword })
      setMessage({ type: 'success', text: res.data.message })
      setTimeout(() => navigate('/vip/dashboard'), 1200)
    } catch (err) {
  ...
  return (
    <div className="max-w-md mx-auto p-6">
      ...
      <h2 className="text-xl font-bold mb-4">Changer le mot de passe - VIP</h2>
